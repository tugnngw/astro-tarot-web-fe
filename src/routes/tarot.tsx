...
import { Send, Sparkles, RotateCcw, Users, Search, ChevronLeft, ChevronRight } from "lucide-react";
...
function TarotPage() {
  ...
  useEffect(() => {
    if (stage === "chat") {
      localStorage.setItem(storageKey(uid), JSON.stringify({ messages, people, drawnCards }));
    }
  }, [messages, stage, uid, people, drawnCards]);
  ...
  const enterChat = () => {
    ...
    const cardLines = drawnCards.map((c, i) => `• ${["Quá khứ", "Hiện tại", "Tương lai"][i]}: ${c} — ${getCardMeaning(c)}`).join("\n");
    setMessages([
      {
        id: "welcome",
        role: "ai",
        content: `Chào ${intro} ✦\n\nDựa trên 3 lá bài bạn vừa rút:\n${cardLines}\n\nMình thấy một dòng năng lượng rõ rệt đang chuyển động quanh bạn. Hãy chia sẻ điều bạn đang băn khoăn — mình sẽ cùng lắng nghe vũ trụ.`,
        ts: Date.now(),
      },
    ]);
    setStage("chat");
  ...
  const send = async (e?: React.FormEvent) => {
    ...
    setMessages((m) => [...m, { id: crypto.randomUUID(), role: "ai", content: MOCK_REPLIES[Math.floor(Math.random() * MOCK_REPLIES.length)], ts: Date.now() }]);
    setThinking(false);
  };
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        {stage === "info" && (
          <div className="glass rounded-2xl p-6">
            ...
            <div className="mb-4">
              <span className="mb-2 flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground"><Users className="h-3.5 w-3.5" /> Số người tham gia</span>
              <div className="flex gap-2">
                {[1, 2, 3, 4].map((n) => (
                  <button key={n} onClick={() => updateCount(n)} className={`flex-1 rounded-lg border py-2 text-sm transition ${count === n ? "border-gold bg-gold/10 text-gold" : "border-border text-muted-foreground hover:border-gold/40"}`}>
                    {n} người
                  </button>
                ))}
              </div>
            ...
            <button onClick={goDraw} className="mt-6 w-full rounded-full bg-gold py-3 font-medium text-primary-foreground glow-gold transition hover:scale-[1.02]">
...
function Dot({ delay = 0 }: { delay?: number }) {
  return <motion.span className="block h-1.5 w-1.5 rounded-full bg-gold" animate={{ opacity: [0.2, 1, 0.2] }} transition={{ duration: 1.2, repeat: Infinity, delay }} />;
}
  ...
  return (
    <div className={`flex ${role === "user" ? "justify-end" : "justify-start"}`}>
      <div className={role === "user"
        ? "max-w-[85%] rounded-2xl rounded-br-sm bg-gold px-4 py-2.5 text-sm text-primary-foreground"
...
function ChatPanel({ messages, thinking, input, setInput, send, resetAll, headerName, lastMsgRef }: ChatPanelProps) {
    ...
    <div className="glass flex h-[calc(100vh-220px)] min-h-[480px] flex-col overflow-hidden rounded-2xl">
      <div className="flex items-center justify-between border-b border-gold/20 px-5 py-3">
        <div className="flex items-center gap-2">
          <div className="grid h-8 w-8 place-items-center rounded-full bg-gold/20 text-gold"><Sparkles className="h-4 w-4" /></div>
          <div>
        ...
        <button onClick={resetAll} className="inline-flex items-center gap-1.5 rounded-full border border-gold/40 px-3 py-1.5 text-xs text-gold transition hover:bg-gold/10">
      ...
      <div className="flex items-center gap-2 border-b border-gold/15 bg-background/30 px-3 py-2">
        <div className="flex flex-1 items-center gap-2 rounded-full border border-gold/30 bg-input/50 px-3">
          ...
          <input value={query} onChange={(e) => onSearch(e.target.value)} placeholder="Tìm trong lịch sử chat…" className="flex-1 bg-transparent py-1.5 text-xs outline-none" />
        </div>
        <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
          <button disabled={pageSafe >= totalPages - 1} onClick={() => setPage((p) => p + 1)} className="rounded p-1 hover:bg-gold/10 disabled:opacity-30"><ChevronLeft className="h-3 w-3" /></button>
          <span>{pageSafe + 1}/{totalPages}</span>
          <button disabled={pageSafe <= 0} onClick={() => setPage((p) => p - 1)} className="rounded p-1 hover:bg-gold/10 disabled:opacity-30"><ChevronRight className="h-3 w-3" /></button>
        </div>
      </div>
        ...
        {pageSafe === 0 && thinking && (
          <div className="flex justify-start">
            <div className="inline-flex items-center gap-2 rounded-2xl rounded-bl-sm border border-gold/25 bg-card/60 px-4 py-2.5 text-sm text-muted-foreground">
              <Dot /><Dot delay={0.15} /><Dot delay={0.3} /><span className="ml-1">Đang kết nối năng lượng…</span>
            </div>
          </div>
        )}
      </div>
      ...
      <form onSubmit={send} className="flex gap-2 border-t border-gold/20 bg-background/30 p-3">
        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Nhập câu hỏi của bạn cho vũ trụ…"
