    ...
    <div className="space-y-4">
        ...
        {stats.isPending ? (
          ...
          <>
            <dl className="mt-4 grid gap-3 sm:grid-cols-4">
              <Stat
                label={`${stats.data!.periodDays} ngày qua`}
                value={String(stats.data!.clicksInPeriod)}
              />
              <Stat
                label="Tổng lượt bấm"
                value={String(stats.data!.totalClicks)}
              />
              <Stat
                label="Đã gắn link"
                value={String(stats.data!.productsWithLink)}
              />
              <Stat
                label="Chưa gắn link"
      ...
      <section className="glass rounded-2xl p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Gắn link tiếp thị của sàn vào từng sản phẩm. Sản phẩm chưa có link
              vẫn hiện nhưng không có nút mua.
            </p>
          </div>
          <button
            type="button"
        ...
        <p className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
          ...
          />
          Chỉ có ẩn, không có xoá. Sản phẩm đã có lượt bấm là số liệu — và những
          lượt bấm đó vẫn có thể đang sinh hoa hồng ở sàn.
        </p>
      </section>
...
function TopProductsChart({ products }: { products: Product[] }) {
  ...
  return (
    <div className="mt-5">
      ...
      <ChartContainer
        config={config}
        className="mt-3 aspect-auto h-[280px] w-full"
      >
        <BarChart
          data={data}
          layout="vertical"
...
function Stat({
  ...
  return (
    <div
      className={`rounded-xl border px-3 py-2 ${warn ? "border-amber-400/40" : "border-gold/20"}`}
    >
      <dt className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
        {label}
      </dt>
      <dd
        className={`mt-1 font-display text-xl ${warn ? "text-amber-300" : "text-gold"}`}
      >
        {value}
      </dd>
    </div>
      ...
      >
        ...
        {product && (
          <p className="mt-1 text-xs text-muted-foreground">
            Đường dẫn <code>{product.slug}</code> giữ nguyên kể cả khi đổi tên —
            đổi slug là làm hỏng mọi link đã chia sẻ.
          </p>
        )}
        ...
        <form onSubmit={submit} className="mt-5 space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field
              ...
              onChange={(v) =>
                setForm((f) => ({
                  ...f,
                  compareAtPrice: v ? Number(v.replace(/[^0-9]/g, "")) : null,
                }))
              }
              inputMode="numeric"
            />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    affiliatePlatform: e.target.value as Platform,
                  }))
                }
                className="mt-1 w-full rounded-lg border border-gold/25 bg-input/70 px-3 py-2 text-sm outline-none focus:border-gold"
              >
            ...
            <Field
              ...
              onChange={(v) =>
                setForm((f) => ({
                  ...f,
                  commissionPercent: Number(v.replace(/[^0-9.]/g, "")) || 0,
                }))
              }
              inputMode="numeric"
            />
          ...
          <p className="text-[11px] text-muted-foreground">
            Danh mục là thứ giữ cho cửa hàng bám đề tài. Món nào không xếp được
            vào năm danh mục này thì gần như chắc chắn không nên bán ở đây.
          </p>
          ...
          <div className="space-y-2 pt-1">
            <Checkbox
              ...
              onChange={(v) =>
                setForm((f) => ({ ...f, imageIsIllustrative: v }))
              }
